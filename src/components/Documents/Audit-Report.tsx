...
import { ModalDocumentsAuditReport } from "../Modals/ModalDocuments";
...
const AuditReport = () => {
	...
	const [editAuditReportDocs, setEditAuditReportDocs] = useState<any>();

	if (isLoading) {
		return <Loading />;
	}
	const handelDelete = async (id: string) => {
		const res = await deleteAuditReport(id);
		if (res?.data?.success) {
	...
	return (
		<>
							...
							<div
								...
								<div className="bg-white absolute bottom-0 w-full p-2 group-hover:p-3 group-hover:h-full transition-all duration-300 h-1/4">
									...
									<div className="mt-4 flex gap-6 justify-center items-center p-4">
										...
										<FilePenLine
											onClick={() => setEditAuditReportDocs(item)}
											size={17}
											color="green"
			...
			{(openModal || !!editAuditReportDocs) && (
				<ModalDocumentsAuditReport
					open={openModal || editAuditReportDocs}
					closed={() => {
						setOpenModal(false);
						setEditAuditReportDocs(null);
						refetch();
					}}
					data={editAuditReportDocs}
				/>
			)}
		</>
	);
