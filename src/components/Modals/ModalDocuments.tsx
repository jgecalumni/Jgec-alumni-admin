...
export const ModalDocumentsKanchenjunga: React.FC<IProps> = memo(
	({ open, closed, data }) => {
		const [previewUrl, setPreviewUrl] = useState<string | null>(null);
		const [addKanchenjungaDocs, { isLoading, isError, error, isSuccess }] =
			useAddKanchenungaDocsMutation();
		const [
			editKanchenjungaDocs,
			{ isLoading: editLoading, isError: editIsError, error: editError },
		] = useUpdateKanchenungaDocsMutation();
		...
		const handelSubmit = async (values: any) => {
			...
			formData.append("file", file);
			if (data) {
				const res = await editKanchenjungaDocs({ formData, id: data.id });
				if (res?.data?.success) {
					toast.success("Document updated successfully");
					closed();
				}
			} else {
				const res = await addKanchenjungaDocs(formData);
				if (res?.data?.success) {
					toast.success("Document added successfully");
					closed();
				}
...
export const ModalDocumentsGivingBack: React.FC<IProps> = memo(
	({ open, closed, data }) => {
		const [previewUrl, setPreviewUrl] = useState<string | null>(null);
		const [addGivingBackDocs, { isLoading, isError, error, isSuccess }] =
			useAddGivingBackDocsMutation();
		const [
			editGivingBackDocs,
			{ isLoading: editLoading, isError: editIsError, error: editError },
		...
		const handelSubmit = async (values: any) => {
			...
			formData.append("file", file);
			if (data) {
				const res = await editGivingBackDocs({ formData, id: data.id });
				if (res?.data?.success) {
					toast.success("Document updated successfully");
					closed();
				}
			} else {
				const res = await addGivingBackDocs(formData);
				if (res?.data?.success) {
					toast.success("Document added successfully");
					closed();
				}
...
export const ModalDocumentsAuditReport: React.FC<IProps> = memo(
	({ open, closed, data }) => {
		const [previewUrl, setPreviewUrl] = useState<string | null>(null);
		const [addAuditReportDocs, { isLoading, isError, error, isSuccess }] =
			useAddauditReportDocsMutation();
		const [
			editAuditReportDocs,
			{ isLoading: editLoading, isError: editIsError, error: editError },
		...
		const handelSubmit = async (values: any) => {
			...
			formData.append("file", file);
			if (data) {
				const res = await editAuditReportDocs({ formData, id: data.id });
				if (res?.data?.success) {
					toast.success("Document updated successfully");
					closed();
				}
			} else {
				const res = await addAuditReportDocs(formData);
				if (res?.data?.success) {
					toast.success("Document added successfully");
					closed();
				}
...
export const ModalDocumentsAgmMom: React.FC<IProps> = memo(
	({ open, closed, data }) => {
		const [previewUrl, setPreviewUrl] = useState<string | null>(null);
		const [addAgmMomDocs, { isLoading, isError, error, isSuccess }] =
			useAddAgmMomDocsMutation();
		const [
			editAgmMomDocs,
			{ isLoading: editLoading, isError: editIsError, error: editError },
		...
		const handelSubmit = async (values: any) => {
			...
			formData.append("file", file);
			if (data) {
				const res = await editAgmMomDocs({ formData, id: data.id });
				if (res?.data?.success) {
					toast.success("Document updated successfully");
					closed();
				}
			} else {
				const res = await addAgmMomDocs(formData);
				if (res?.data?.success) {
					toast.success("Document added successfully");
					closed();
				}
